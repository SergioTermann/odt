                ...
                if machine.is_activated:
                    if machine.bounding_boxe is not None:
                        if machine.nationality == 1:
                            ...
                            # Overlays.add_line(machine.bound_front * aX, (machine.bound_front + hg.Vec3(0, 0, 1)) * aX, hg.Color.Green, hg.Color.Green)
                            # Overlays.add_line(machine.bound_back * aX, (machine.bound_back + hg.Vec3(0, 0, -1)) * aX, hg.Color.Green, hg.Color.Green)
                            # Overlays.add_line(machine.bound_up * aY, (machine.bound_up + hg.Vec3(0, 1, 0)) * aY, hg.Color.Green, hg.Color.Green)
                            # Overlays.add_line(machine.bound_down * aY, (machine.bound_down + hg.Vec3(0, -1, 0)) * aY, hg.Color.Green, hg.Color.Green)
                            # Overlays.add_line(machine.bound_right * aZ, (machine.bound_right + hg.Vec3(1, 0, 0)) * aZ, hg.Color.Green, hg.Color.Green)
                            # Overlays.add_line(machine.bound_left * aZ, (machine.bound_left + hg.Vec3(-1, 0, 0)) * aZ, hg.Color.Green, hg.Color.Green)
                            Overlays.display_boxe(machine.get_world_bounding_boxe(), hg.Color.Blue)
                        else:
                            Overlays.display_boxe(machine.get_world_bounding_boxe(), hg.Color.Red)
